...
.DESCRIPTION
    Extract the complete list of All Computers and Servers.
.EXAMPLE
    PS C:\adrt> .\ad-inventory.ps1
.NOTES
    Name: Thaigo Freitas
...
$path = "ad-reports\ad-inventory"
$html = "$path\ad-inventory-$date.html"
$csv = "$path\ad-inventory-$date.csv"

#-- All Computers and Servers
$t_cs = (Get-ADComputer -Filter 'Name -like "*"' -Properties *).count
...
#-- Show Total
$table += "<center><h3><b>Computers and Servers: <font color=red>$t_cs</font></b></h3></center>"

#-- Filter
$cs = @(Get-ADComputer -Filter 'Name -like "*"' -Properties *)

$result = @($cs | Select-Object Name, IPv4Address, LastlogonDate, OperatingSystem, Description)

#-- Order by (A-Z)
$result = $result | Sort "OperatingSystem"
...
$title=
		...
		<tr>
		<td bgcolor='#F9F9F9'>
		<font face='Calibri' size='5px'><b>Active Directory - Inventory</b></font>
...
$Subject = "[ Report-$mounth ] Active Directory - Inventory"
