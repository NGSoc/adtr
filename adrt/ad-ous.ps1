...
.DESCRIPTION
    Extract the complete list of All OUs.
.EXAMPLE
    PS C:\adrt> .\ad-ous.ps1
.NOTES
    Name: Thaigo Freitas
...
$path = "ad-reports\ad-ous"
$html = "$path\ad-ous-$date.html"
$csv = "$path\ad-ous-$date.csv"
...
#-- Show Total
$table += "<center><h3><b>Total OUs: <font color=red>$t_ous</font></b></h3></center>"

#-- Filter
$ous = @(Get-ADOrganizationalUnit -Filter {name -like "*"} -Properties Description | Select Name, Description, DistinguishedName)

$result = @($ous | Select-Object Name, Description, DistinguishedName)
...
$title=
		...
		<tr>
		<td bgcolor='#F9F9F9'>
		<font face='Calibri' size='5px'><b>Active Directory - All OUs</b></font>
...
$message = $message + "<tr><td height='40'>Total OUs</td><td>$t_ous</td></tr>"
...
$Subject = "[ Report-$mounth ] Active Directory - All OUs"
