...
$path = "ad-reports\ad-computers"
$html = "$path\ad-computers-$date.html"
$csv = "$path\ad-computers-$date.csv"
...
#-- Show Total
$table += "<center><h3><b>Total Computers: <font color=red>$t_c</font></b></h3></center>"

#-- Filter
$computers = @(Get-ADComputer -Filter { OperatingSystem -NotLike '*Windows Server*' } -Properties OperatingSystem, Description)

$result = @($computers | Select-Object Name, OperatingSystem, Description)
...
$title=
		...
		<tr>
		<td bgcolor='#F9F9F9'>
		<font face='Calibri' size='5px'><b>Active Directory - All Computers</b></font>
...
$Subject = "[ Report-$mounth ] Active Directory - All Computers"
